...
int Algorithms::findNearestNeighborClusterRoute(const int& city, const vector<vector<float>>& distances, vector<bool>& visited, const vector<int>& route, const unordered_map<int, float>& repToClusterCosts){
    int nearestNeighbor = -1;
    float shortestDistance = INFINITY_FLOAT;

    for (const int& index : route) {
        bool isIndexClusterRep = repToClusterCosts.contains(index);
        float dist = isIndexClusterRep ? distances[city][index] + repToClusterCosts.at(index) : distances[city][index];
        if (index != city && !visited[index] && dist < shortestDistance) {
...
vector<int> Algorithms::nearestNeighborClusterRoute(const int& initialCity, const vector<vector<float>>& distances, float& tourCost, const vector<int>& route, const unordered_map<int, float>& repToClusterCosts) {
    int numCities = static_cast<int>(route.size());
    ...
    while (path.size() < numCities) {
        int nearestNeighbor = findNearestNeighborClusterRoute(currentCity, distances, visited, route, repToClusterCosts);
        bool isNeighborClusterRep = repToClusterCosts.contains(nearestNeighbor);
        visited[nearestNeighbor] = true;
        path.push_back(nearestNeighbor);
        tourCost += isNeighborClusterRep ? distances[currentCity][nearestNeighbor] + repToClusterCosts.at(nearestNeighbor) : distances[currentCity][nearestNeighbor];
        currentCity = nearestNeighbor;
    }

    bool isInitialClusterRep = repToClusterCosts.contains(initialCity);
    tourCost += isInitialClusterRep ? distances[currentCity][initialCity] + repToClusterCosts.at(initialCity) : distances[currentCity][initialCity];
    path.push_back(initialCity);
 ...
 */
vector<int> Algorithms::multiStartTwoOptNearestNeighbor(float &cost, const vector<vector<float>> &distances, const int& numOfStarts, const vector<int>& cluster, const unordered_map<int, float>& repToClusterCosts) {
    ...
    for (const int& startingPoint : startingPoints) {
        ...
        vector<int> tspSolution = repToClusterCosts.empty() ? nearestNeighborTSP(startingPoint, distances, solutionCost, cluster) : nearestNeighborClusterRoute(startingPoint, distances, solutionCost, cluster, repToClusterCosts);
...
double Algorithms::getClusteringScore(const vector<vector<int>>& clusters) {
    ...
    for (const vector<int>& cluster : clusters) {
        totalElements += static_cast<double>(cluster.size());
    }
    double avgElements = totalElements / numClusters;

    double totalDeviation = 0;
    for (const vector<int>& cluster : clusters) {
        totalDeviation += abs(avgElements - static_cast<double>(cluster.size()));
    }
    double avgDeviation = totalDeviation / numClusters;
    double deviationScore = avgDeviation == 0 ? 1 : (1.0 / avgDeviation);

    double score = numClusters * pow(totalElements, 2) * deviationScore;
...
vector<int> Algorithms::customTspHeuristic(const vector<vector<float>> &distances, const float &clusteringEpsilon, const int &clusteringMinPts, float& resultCost) {
    ...
    unordered_map<int, float> repToClusterCosts;
    unordered_map<int, vector<int>> repToClusterSol;
    for (int i = 0; i < clusters.size(); i++) {
        vector<int> cluster = clusters[i];
        int representative = clusterRepresentatives[i];
        float cost;
        vector<int> sol = multiStartTwoOptNearestNeighbor(cost, distances, static_cast<int>(cluster.size()), cluster);

        int last = static_cast<int>(sol.size()) - 1;
        repToClusterCosts[representative] = cost - distances[sol[last - 1]][sol[last]];
        rotateCycle(sol, representative);
        repToClusterSol[representative] = sol;
    }
    ...
    vector<int> reducedTspInstanceSolution = Algorithms::multiStartTwoOptNearestNeighbor(reducedTspInstanceCost, distances, static_cast<int>(reducedTspInstance.size()),reducedTspInstance, repToClusterCosts);

    int n = static_cast<int>(reducedTspInstanceSolution.size());
    vector<int> finalSolution;
    finalSolution.reserve(distances.size());
    finalSolution.push_back(reducedTspInstanceSolution[0]);

    for (int i = 1; i < n; i++) {
        int currVertice = reducedTspInstanceSolution[i];
        bool isCluster = repToClusterSol.contains(currVertice);
        if (isCluster) {
            vector<int> clusterSol = repToClusterSol[currVertice];
            finalSolution.insert(finalSolution.end(), make_move_iterator(clusterSol.begin()), make_move_iterator(clusterSol.end() - 1));
        } else {
            finalSolution.push_back(currVertice);
        }
    }

    bool firstWasCluster = repToClusterSol.contains(reducedTspInstanceSolution[0]);
    if (firstWasCluster) {
...
void Algorithms::rotateCycle(vector<int>& cycle, const int& start) {
    auto it = std::find(cycle.begin(), cycle.end(), start);
    if (it != cycle.end()) {
        std::rotate(cycle.begin(), it, cycle.end() - 1);
